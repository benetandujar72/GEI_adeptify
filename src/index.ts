...
import { initializeDatabase, sql } from './database/init.js';
  ...
  if (!staticPath) {
    ...
    app.get('*', (_req, res) => {
      ...
      if (!_req.path.startsWith('/api/')) {
        ...
        logger.info(`📂 Ruta solicitada: ${_req.path}`);
...
app.get('/api/debug-manifest', (_req, res) => {
...
if (process.env.NODE_ENV === 'production') {
  ...
  app.get('*', (_req, res, next) => {
    logger.info(`🔍 Petición recibida: ${_req.method} ${_req.path}`);
    logger.info(`📋 Headers: ${JSON.stringify(_req.headers, null, 2)}`);
    
    // No interceptar rutas de API
    if (_req.path.startsWith('/api/')) {
      logger.info(`✅ Ruta API detectada, pasando al siguiente middleware: ${_req.path}`);
      return next();
    }
    
    // No interceptar archivos estáticos
    if (_req.path.includes('.') && !_req.path.includes('..')) {
      logger.info(`✅ Archivo estático detectado, pasando al siguiente middleware: ${_req.path}`);
      return next();
    }
    
    // Para todas las demás rutas, servir index.html (SPA)
    const indexPath = path.join(__dirname, '../client/dist/index.html');
    logger.info(`🌐 Sirviendo index.html para ruta SPA: ${_req.path}`);
